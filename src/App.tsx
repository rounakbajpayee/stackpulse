...
export const App: React.FC = () => {
  const [startups, setStartups] = useState<Startup[]>(DEFAULT_STARTUPS as any);
    ...
    async function loadData() {
      try {
        ...
        if (data && data.length > 0 && !error) {
          ...
          // Fallback to local verified startups
          setStartups(DEFAULT_STARTUPS as any);
        }
      } catch (err) {
  ...
  // Sync real live batch from Python crawler / backend API
  const handleSync = async () => {
    ...
    try {
      ...
      console.log('Using direct client-side live enrichment sync...');
    }

    // Client-side live ingestion fallback from Hacker News AI launches
    try {
      const hnRes = await fetch(
        'https://hn.algolia.com/api/v1/search_by_date?tags=show_hn&query=AI&hitsPerPage=6'
      );
      ...
      const newItems: Startup[] = (hnData.hits || []).map((hit: any, i: number) => {
        ...
        return {
          ...
          migration_opportunity_score: isSb ? '15%' : `${85 + (i % 10)}%`,
          framework: 'Next.js',
      ...
      if (newItems.length > 0) {
        setStartups([...newItems, ...(DEFAULT_STARTUPS as any)]);
      }
    } catch (err) {
