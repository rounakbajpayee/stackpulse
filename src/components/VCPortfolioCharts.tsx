    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {cohorts.map((cohort) => {
          ...
          const sbCount = matched.filter(s => (s.database_stack || '').toLowerCase().includes('supabase')).length;
          const pgCount = matched.filter(s => (s.database_stack || '').toLowerCase() === 'postgresql' || (s.database_stack || '').toLowerCase() === 'postgres').length;
          const mysqlCount = matched.filter(s => (s.database_stack || '').toLowerCase().includes('mysql')).length;
          ...
          const otherCount = Math.max(0, cohortTotal - sbCount - pgCount - mysqlCount - fbCount - mongoCount - dynamoCount - psCount);

          const sbPct = Math.round((sbCount / cohortTotal) * 100);
          const pgPct = Math.round((pgCount / cohortTotal) * 100);
          const mysqlPct = Math.round((mysqlCount / cohortTotal) * 100);
          ...
          const otherPct = Math.max(0, 100 - sbPct - pgPct - mysqlPct - fbPct - mongoPct - dynamoPct - psPct);
            ...
            >
              <div>
                ...
                <div className="h-3.5 w-full rounded-full bg-[#0B0F19] overflow-hidden flex mb-3 p-0.5 border border-[#1F2937]">
                  ...
                  <div style={{ width: `${pgPct}%` }} className="bg-[#336791]" title={`PostgreSQL: ${pgPct}%`}></div>
                  <div style={{ width: `${mysqlPct}%` }} className="bg-[#E48E00]" title={`MySQL: ${mysqlPct}%`}></div>
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px] mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-slate-300">
                      <span className="w-2 h-2 rounded-full bg-[#336791]"></span>
                      <span>PostgreSQL</span>
                    </span>
                    <span className="font-semibold text-[#336791]">{pgPct}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-slate-300">
                      <span className="w-2 h-2 rounded-full bg-[#E48E00]"></span>
                      <span>MySQL</span>
                    </span>
                    <span className="font-semibold text-[#E48E00]">{mysqlPct}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-1.5 text-slate-300">
