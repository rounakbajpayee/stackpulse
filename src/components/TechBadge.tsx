...
export const TechBadge: React.FC<TechBadgeProps> = ({ tech, isVector }) => {
  if (!tech || tech === 'None' || tech === 'Unknown') {
    return (
  ...
  if (t.includes('supabase')) {
    ...
    colorClasses = 'bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800/60';
  }

  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium border ${colorClasses}`}>
      {tech}
    </span>
  );
};
